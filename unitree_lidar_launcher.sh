...
# 脚本功能：一键启动 MID360 雷达 + FAST-LIVO2（不使用重定位）
# 使用说明：1. 赋予执行权限  2. 直接运行脚本

# 定义工作空间根目录和命令（请确认路径与你的实际路径一致！）
WORKSPACE="/home/r1/Unitree_lidar/FAST_LIVO2_ROS2_relocation_ultra"
CMD1="source install/setup.bash && ros2 launch livox_ros_driver2 msg_MID360_launch.py"
CMD2="source install/setup.bash && ros2 launch fast_livo mapping_avia.launch.py use_rviz:=True"
SERIAL_WORKSPACE="/home/r1/Unitree_lidar/serial_driver_ros2"
CMD3="source install/setup.bash && ros2 launch serial_driver serial_driver.launch.py"
...
# 启动第三个终端：运行串口数据发送
echo "启动终端3：serial_driver_ros2..."
gnome-terminal --title="serial_driver_ros2" --working-directory="$SERIAL_WORKSPACE" -- bash -c "$CMD3; exec bash"
